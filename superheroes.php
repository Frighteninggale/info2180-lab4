...
$search_term = strtolower(trim(filter_var($query, FILTER_SANITIZE_FULL_SPECIAL_CHARS)));

$found_superhero = null;

if (!empty($search_term)) {
    foreach ($superheroes as $hero) {
        $name_match = strtolower($hero['name']) === $search_term;
        $alias_match = strtolower($hero['alias']) === $search_term;
        
        if ($name_match || $alias_match) {
            $found_superhero = $hero;
            break; 
        }
    }
    ...
    if ($found_superhero) {
        ?>
        <h2><?= strtoupper($found_superhero['alias']); ?></h2>
        <h3>A.K.A. <?= $found_superhero['name']; ?></h3>
        <p><?= $found_superhero['biography']; ?></p>
        <?php
    } else {
        ?>
        <span class='not-found'>SUPERHERO NOT FOUND</span>
        <?php
    }
